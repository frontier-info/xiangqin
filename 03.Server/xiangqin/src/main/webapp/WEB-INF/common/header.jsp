<%@ page language="java" contentType="text/html; charset=utf-8"
	pageEncoding="utf-8"%>
   <nav class="navbar navbar-primary" role="navigation">
    <div class="container-fluid">
     <div class="navbar-header">
      <a class="navbar-brand" href="${pageContext.request.contextPath}/ui/s002001">XiangQin</a>
     </div>
     <div>
      <!--向右对齐-->
      <ul class="nav nav-pills navbar-right">
       <li class="nav-item">
        <a href="${pageContext.request.contextPath}/ui/s002001" class="nav-link">首页</a>
       </li>
       <li class="nav-item">
        <a href="${pageContext.request.contextPath}/ui/s002001/e001" class="nav-link">用户信息设定</a>
       </li>
       <li class="nav-item">
        <a href="${pageContext.request.contextPath}/ui/s002001/e001" class="nav-link">检索条件设定</a>
       </li>
       <li class="nav-item">
        <a href="${pageContext.request.contextPath}/ui/s003001/e001" class="nav-link">我打过招呼的人</a>
       </li>
       <li class="nav-item">
        <a href="${pageContext.request.contextPath}/ui/s002001" class="nav-link">向我打招呼的人</a>
       </li>
       <li class="nav-item">
        <a href="${pageContext.request.contextPath}/ui/s002001/e003" class="nav-link">退出登录</a>
       </li>
      </ul>
     </div>
    </div>
   </nav>

   